...
const ShowcaseCarousel = () => {
  ...
  const slides = [
    {
      title: "Input Mágico",
      mockup: (
        <div className="w-full h-full bg-white rounded-2xl border border-gray-200 shadow-lg p-4 flex flex-col">
          <div className="flex items-end gap-2 mb-4">
            ...
            <div className="bg-gray-100 rounded-xl rounded-bl-none p-3 text-xs text-left text-gray-800 leading-snug font-medium">
          ...
          <div className="mt-auto border border-gray-100 rounded-xl p-3 bg-white shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)]">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="font-bold text-sm text-black">Llamar al dentista</span>
            </div>
    ...
    },
    {
      ...
      mockup: (
        <div className="w-full h-full bg-white rounded-2xl border border-gray-200 shadow-lg p-5 flex flex-col">
          ...
          <div className="space-y-4 flex-1 flex flex-col justify-center">
            {[
              ...
              <div key={i} className="flex items-center justify-between">
                <span className="text-xs font-bold text-gray-700 w-20 text-left truncate">{h.n}</span>
                <div className="flex gap-1">
                  {h.p.map((done, d) => (
                    <div key={d} className={`w-3.5 h-3.5 rounded-[4px] ${done ? 'bg-black' : 'bg-gray-100'}`} />
                  ))}
                </div>
              </div>
            ))}
      ...
      mockup: (
        <div className="w-full h-full bg-white rounded-2xl border border-gray-200 shadow-lg p-5 flex flex-col">
          ...
          <div className="relative flex-1 border-l-2 border-gray-100 ml-2 pl-4 space-y-3 py-1">
            ...
            <div className="bg-black p-3 rounded-xl text-left shadow-md">
  ...
  return (
    <div className="relative w-full h-[300px] md:h-[400px] flex items-center justify-center overflow-hidden bg-[#FAFAFA]">
      {isMobile && (
        <motion.div drag="x" dragConstraints={{ left: 0, right: 0 }} onDragEnd={handleDragEnd} className="absolute inset-0 z-40 cursor-grab active:cursor-grabbing" />
      )}

      <div className="relative w-full max-w-3xl h-full flex items-center justify-center [perspective:1000px] pointer-events-none md:pointer-events-auto">
        {slides.map((slide, index) => {
          ...
          return (
            <div
              key={index}
              className={cn(
                'absolute w-[220px] h-[300px] md:w-[260px] md:h-[360px] transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)]',
                'flex flex-col items-center justify-center'
              )}
              style={{
                transform: `translateX(${pos * 65}%) scale(${isCenter ? 1 : isAdjacent ? 0.85 : 0.7}) rotateY(${pos * -15}deg)`,
      ...
      {!isMobile && (
        <>
          <button onClick={handlePrev} className="absolute left-4 top-1/2 -translate-y-1/2 rounded-full h-10 w-10 z-30 bg-white/80 backdrop-blur-md border border-gray-200 shadow-sm flex items-center justify-center text-black hover:bg-gray-50 transition-colors">
            <ChevronLeft className="h-5 w-5" />
          </button>
          <button onClick={handleNext} className="absolute right-4 top-1/2 -translate-y-1/2 rounded-full h-10 w-10 z-30 bg-white/80 backdrop-blur-md border border-gray-200 shadow-sm flex items-center justify-center text-black hover:bg-gray-50 transition-colors">
...
export default function Landing() {
  ...
  return (
    <div className="min-h-[100dvh] bg-[#FAFAFA] flex flex-col selection:bg-black selection:text-white overflow-x-hidden font-sans">
      ...
      {/* HEADER STICKY */}
      <header className="fixed top-0 left-0 right-0 px-6 py-3 flex items-center justify-between w-full z-50 bg-[#FAFAFA]/80 backdrop-blur-md border-b border-gray-200/50">
        <div className="w-full max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            {!isMobile && (
              <Link href="/signup">
                <button className="bg-black text-white text-sm font-semibold px-4 py-2 rounded-full hover:bg-gray-800 transition-transform active:scale-95 shadow-sm">
                  Empezar
                </button>
              </Link>
            )}
      ...
      <main className="flex-1 flex flex-col items-center w-full pt-20">
        ...
        {/* HERO SECTION (2 Columnas en PC, Apilado en Móvil) */}
        <section className="w-full max-w-6xl mx-auto px-6 pt-6 pb-12 md:py-20">
          <div className="flex flex-col md:flex-row items-center justify-between gap-12 md:gap-12">

            {/* Columna Izquierda: Textos */}
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, ease: "easeOut" }} className="w-full md:w-1/2 text-center lg:text-left">
              <h1 className="text-5xl md:text-6xl font-bold tracking-tighter text-black mb-4 leading-[1.05]">
                Ponlo y hazlo.
              </h1>
              <p className="text-lg md:text-xl text-gray-500 mb-8 font-medium leading-tight max-w-md">
        ...
        {/* NUESTRA HISTORIA (Escalera en PC, Apilado en Móvil) */}
        <section className="w-full max-w-5xl mx-auto px-6 py-16 md:py-24">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.6 }}>
            <h2 className="text-3xl md:text-5xl font-bold tracking-tighter text-black mb-12 text-left">
              Por qué creé <span className="text-gray-400">do it!</span>
            </h2>

            <div className="flex flex-col gap-6">
              {/* Tarjeta 1 */}
              <div className="bg-white rounded-2xl p-6 md:p-8 border border-gray-200 shadow-sm w-4/5 md:w-3/4 self-start">
              ...
              {/* Tarjeta 2 */}
              <div className="bg-white rounded-2xl p-6 md:p-8 border border-gray-200 shadow-sm w-4/5 md:w-3/4 self-center">
                <p className="text-base md:text-lg text-gray-600 font-medium leading-relaxed">
                  Las apps del mercado estaban llenas de colores chillones, gráficos innecesarios y formularios complejos que me hacían perder el tiempo.
                </p>
              </div>

              {/* Tarjeta 3 */}
              <div className="bg-white rounded-2xl p-6 md:p-8 border border-gray-200 shadow-sm w-4/5 md:w-3/4 self-end">
                <p className="text-base md:text-lg text-gray-600 font-medium leading-relaxed">
                  Quería algo donde pudiera entrar, soltar lo que tenía en la cabeza y salir en 3 segundos, sabiendo que una IA lo organizaría todo por mí.
                </p>
                <p className="text-base md:text-lg text-black font-semibold mt-4">
                  Como no existía, la construí.
                </p>
              </div>
            </div>
          </motion.div>
        ...
        <section className="w-full max-w-5xl mx-auto px-6 py-12">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.6 }}>
            <div className="text-center mb-10">
              <h2 className="text-3xl md:text-4xl font-bold tracking-tighter text-black mb-2">Todo lo que necesitas.</h2>
              <p className="text-base text-gray-500 font-medium">Nada de lo que no.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {/* Tarjeta Input Mágico */}
              <div className="md:col-span-2 bg-white rounded-3xl p-6 md:p-8 border border-gray-200 shadow-sm flex flex-col gap-2">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-gray-50 rounded-xl flex items-center justify-center border border-gray-100 shrink-0"><Mic className="w-5 h-5 text-black" /></div>
                  <h3 className="text-lg font-bold text-black tracking-tight">Input Mágico</h3>
                </div>
              ...
              <div className="bg-white rounded-3xl p-6 border border-gray-200 shadow-sm flex flex-col justify-center">
                <div className="flex items-center gap-3 mb-2">
                  <div className="w-10 h-10 bg-gray-50 rounded-xl flex items-center justify-center border border-gray-100"><Calendar className="w-5 h-5 text-black" /></div>
                  <h3 className="text-lg font-bold text-black tracking-tight">Calendario</h3>
                </div>
              ...
              <div className="bg-white rounded-3xl p-6 border border-gray-200 shadow-sm flex flex-col justify-center">
                <div className="flex items-center gap-3 mb-2">
                  <div className="w-10 h-10 bg-gray-50 rounded-xl flex items-center justify-center border border-gray-100"><Target className="w-5 h-5 text-black" /></div>
                  <h3 className="text-lg font-bold text-black tracking-tight">Hábitos</h3>
                </div>
                <p className="text-gray-500 font-medium text-sm">Tracker visual y rachas.</p>
              </div>

              {/* Tarjeta Offline */}
              <div className="md:col-span-2 bg-black rounded-3xl p-6 md:p-8 border border-black shadow-md flex flex-col gap-2">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center border border-white/10 shrink-0"><Zap className="w-5 h-5 text-white" /></div>
                  <h3 className="text-lg font-bold text-white tracking-tight">Rendimiento Offline</h3>
                </div>
                <p className="text-gray-400 font-medium text-sm">Diseñada como PWA. Funciona sin conexión, se instala en tu móvil y responde en 0 milisegundos.</p>
              </div>
            </div>
          </motion.div>
        </section>
        ...
        <section className="w-full max-w-4xl mx-auto px-6 py-16 border-t border-gray-200/50">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.6 }}>
            <div className="text-center mb-12">
              <h2 className="text-3xl md:text-4xl font-bold tracking-tighter text-black mb-2">Empieza gratis.</h2>
              <p className="text-base text-gray-500 font-medium">Mejora cuando lo necesites.</p>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-8 md:gap-6 mt-8">
              ...
              {/* Premium Plan */}
              <div className="bg-black rounded-[2rem] p-8 border border-black shadow-xl flex flex-col mt-8 md:mt-0">
                {/* Switch de precios centrado en móvil, izquierda en PC */}
                <div className="flex justify-center -mt-12 mb-6 z-10">
                  <div className="bg-white border border-gray-200 p-1 rounded-full flex items-center shadow-md w-max">
      ...
      <Dialog open={showIOSAlert} onOpenChange={setShowIOSAlert}>
        <DialogContent className="bg-white rounded-3xl p-6 sm:max-w-md border-0 shadow-2xl [&>button]:hidden">
          ...
          <button onClick={() => setShowIOSAlert(false)} className="w-full mt-4 bg-black text-white rounded-xl py-3 font-medium">Entendido</button>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showIOSInstructions} onOpenChange={setShowIOSInstructions}>
        <DialogContent className="bg-white rounded-3xl p-6 sm:max-w-md border-0 shadow-2xl [&>button]:hidden">
          ...
          <button onClick={() => setShowIOSInstructions(false)} className="w-full mt-4 bg-black text-white rounded-xl py-3 font-medium">Entendido</button>
        </DialogContent>
      </Dialog>
    </div>
  );
