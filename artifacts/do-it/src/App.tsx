...
const queryClient = new QueryClient({
  defaultOptions: {
    queries: { gcTime: 1000 * 60 * 60 * 24 * 7, staleTime: 0, refetchOnWindowFocus: true },
  },
});

const asyncStoragePersister = createAsyncStoragePersister({
  storage: { getItem: async (key) => await get(key), setItem: async (key, value) => await set(key, value), removeItem: async (key) => await del(key) },
});
...
function ProtectedRoute({ component: Component, ...rest }: { component: any, [key: string]: any }) {
  ...
  useEffect(() => { if (!loading && !session) setLocation("/login"); }, [session, loading, setLocation]);
...
function Router() {
  return (
    <Switch>
      ...
      <Route path="/signup" component={Signup} /> {/* <-- NUEVO */}
